...
Basically `F#` makes no distinction between pure and impure functions therefore the compiler cannot memoize the function.
Haskell, for example, handles effects differently, and functions aren't always evaluated when given an argument.
