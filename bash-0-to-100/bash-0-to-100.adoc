...
found myself looking that up every time.
And I had questions like _why can't I use a variable in my if statements_
It took me a while to cotton on to the underlying patterns. +

This guide attempts to provide a sane and consistent viewpoint on how to use it.
It may not be the best most fancy way of doing things, but it's consistent.
If you want fancy then see
https://www.tldp.org/LDP/abs/html/sha-bang.html[advanced bash programming].
It's advanced but overwhelming and unstructured. +
...
Placing the `$` in front of a parameter causes *parameter expansion*.

TIP: Best practice is to use double quotes when performing parameter expansion.
